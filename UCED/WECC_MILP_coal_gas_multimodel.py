...
model.on = Var(model.UC,model.HH_periods, within=Binary, initialize=0)

#1 if unit is switching on in hour i, otherwise 0
model.switch = Var(model.UC,model.HH_periods, within=Binary,initialize=0)
...
def SysCost(model):
    ...
    return fixed + starts + gen + slack + hydro_cost + wind_cost + solar_cost + exchange_cost + offshorewind_cost + powerflow_cost
...
model.SwitchConstraint = Constraint(model.UC,model.hh_periods,rule = SwitchCon)
...
model.MinimumUp = Constraint(model.UC,model.HH_periods,model.HH_periods,rule=MinUp)
...
def MinDown(model,j,i,k):
    if i > 0 and k > i and k < min(i+model.mindn[j]-1,model.HorizonHours):
        ...
        return Constraint.Skip
model.MinimumDown = Constraint(model.UC,model.HH_periods,model.HH_periods,rule=MinDown)
...
model.MaxCapC= Constraint(model.UC,model.hh_periods,rule=MaxCC)


def MinC(model,j,i):
    return model.mwh[j,i] >= model.on[j,i] * model.mincap[j]
model.MinCap= Constraint(model.UC,model.hh_periods,rule=MinC)
